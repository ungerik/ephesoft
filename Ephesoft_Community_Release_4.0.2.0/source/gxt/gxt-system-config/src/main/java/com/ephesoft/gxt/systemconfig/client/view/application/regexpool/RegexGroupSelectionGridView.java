...
import com.ephesoft.gxt.systemconfig.client.presenter.application.regexpool.RegexGroupSelectionGridPresenter;
 ...
 */
public class RegexGroupSelectionGridView extends SystemConfigInlineView<RegexGroupSelectionGridPresenter> implements HasResizableGrid {
	...
	protected SystemConfigGrid<RegexGroupDTO> regexGroupGrid;
	...
	public RegexGroupSelectionGridView() {
		...
		regexGroupGrid.addStyleName("systemConfigRegexGrid");
	...
	public void initialize() {
		regexGroupGrid = new SystemConfigGrid<RegexGroupDTO>(PropertyAccessModel.REGEX_GROUP) {

			@Override
			public void completeEditing(CompleteEditEvent<RegexGroupDTO> completeEditEvent) {
				regexGroupGrid.getStore().commitChanges();
				int index = completeEditEvent.getEditCell().getRow();
				RegexGroupDTO editedRegexGroup = regexGroupGrid.getModel(index);
				presenter.editRegexGroupEvent(editedRegexGroup);
				if (regexGroupGrid.isValid(editedRegexGroup)) {
					presenter.updateRegexGroup(editedRegexGroup);
				}

				regexGroupGrid.refreshGrid(editedRegexGroup, false);
				presenter.checkNewGroupExist(regexGroupGrid.getStore().getAll());
			}
		};
		...
		regexGroupGrid.setIdProvider(RegexGroupProperties.INSTANCE.name());
	}
	...
	public void refreshGrid() {
		regexGroupGrid.refreshGrid(true);
	}
}

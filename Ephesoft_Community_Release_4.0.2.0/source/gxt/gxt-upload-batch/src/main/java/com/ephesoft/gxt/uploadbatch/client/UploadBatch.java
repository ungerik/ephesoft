...
/**
 * Upload Batch Screen entry point class.
 * 
 * @author Ephesoft
 *
 */
public class UploadBatch extends DCMAEntryPoint<UploadBatchServiceAsync> {
	...
	@Override
	public void onLoad() {
		final UploadBatchController controller = new UploadBatchController(eventBus, rpcService);
		RootPanel.get().add(controller.createView());
//		eventBus.fireEvent(new UploadBatchViewAdditionEvent());
	}
	...
	public LocaleInfo createLocaleInfo(String locale) {
		return new LocaleInfo(locale, UploadBatchConstants.UPLOAD_BATCH_CONSTANTS, UploadBatchConstants.UPLOAD_BATCH_MESSAGES);
	}

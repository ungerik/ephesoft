 ...
 */
public class UniqueBCFieldNameValidator<T extends BatchClassFieldDTO, V> implements Validator<BatchClassFieldDTO> {
	...
	public boolean validate(List<BatchClassFieldDTO> listOfModels, BatchClassFieldDTO model,
		...
		if (null != listOfModels && !listOfModels.isEmpty() && null != model && null != valueProvider && model.getName() != null
			...
			for (BatchClassFieldDTO fieldDTOModel : listOfModels) {
				if (model != fieldDTOModel && model.getName().trim().equals(fieldDTOModel.getName())) {
					valid = false;
					break;
				}
			}
	...
	public String getErrorMessage() {
		return "Field name should be unique and non empty";

...
public class SSODatabaseAuthenticator implements AuthenticationService {
	...
	public String logout(final HttpServletRequest httpServletRequest) {
		...
		if (null != httpServletRequest) {
			...
			requestURL = RequestUtil.getSessionAttribute(httpServletRequest, CoreCommonConstant.LOGOUT_URL_ATTRIBUTE_KEY);
			session.invalidate();
	...
	public void setUserDetails(final HttpServletRequest httpServletRequest, final FilterConfig filterConfig) {
		...
		if (null == httpServletRequest || null == filterConfig) {
			...
			if (!RequestUtil.isSessionAttribute(httpServletRequest, CoreCommonConstant.USERNAME_ATTRIBUTE_KEY)) {
				final String userIdHeaderName = filterConfig.getInitParameter(CoreCommonConstant.REQUEST_USERNAME_HEADER);
				userName = RequestUtil.getAttributeFromHeader(httpServletRequest, userIdHeaderName);
				RequestUtil.setSessionAttribute(httpServletRequest, CoreCommonConstant.USERNAME_ATTRIBUTE_KEY, userName);
			}

			// Add Logout URL to session
			String logoutUrl = null;
			if (!RequestUtil.isSessionAttribute(httpServletRequest, CoreCommonConstant.LOGOUT_URL_ATTRIBUTE_KEY)) {
				logoutUrl = filterConfig.getInitParameter(CoreCommonConstant.LOGOUT_URL_PARAM_NAME);
				RequestUtil.setSessionAttribute(httpServletRequest, CoreCommonConstant.LOGOUT_URL_ATTRIBUTE_KEY, logoutUrl);
			}

			// Add Group Name
			String groupName = null;
			if (!RequestUtil.isSessionAttribute(httpServletRequest, CoreCommonConstant.GROUP_NAME_ATTRIBUTE_KEY)) {
				final String groupNameHeader = filterConfig.getInitParameter(CoreCommonConstant.GROUP_NAME_PARAM_NAME);
				groupName = RequestUtil.getAttributeFromHeader(httpServletRequest, groupNameHeader);
				RequestUtil.setSessionAttribute(httpServletRequest, CoreCommonConstant.GROUP_NAME_ATTRIBUTE_KEY, groupName);
			}

			// Add Super-Admin
			Boolean isSuperAdmin = Boolean.FALSE;
			if (!RequestUtil.isSessionAttribute(httpServletRequest, CoreCommonConstant.SUPER_ADMIN_ATTRIBUTE_KEY)) {
				final String isSuperAdminHeader = filterConfig.getInitParameter(CoreCommonConstant.SUPER_ADMIN_PARAM_NAME);
				...
				RequestUtil.setSessionAttribute(httpServletRequest, CoreCommonConstant.SUPER_ADMIN_ATTRIBUTE_KEY, isSuperAdmin);
			}

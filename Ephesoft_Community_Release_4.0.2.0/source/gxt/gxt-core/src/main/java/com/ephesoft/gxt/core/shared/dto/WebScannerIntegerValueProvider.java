 ...
 */
public class WebScannerIntegerValueProvider<T extends WebScannerConfigurationDTO, V extends Comparable<Integer>> implements
	 ...
	 */
	public WebScannerIntegerValueProvider(String headerName, Class<V> className) {
		super();
				...
				for (WebScannerConfigurationDTO webScannerDTO : childList) {
					if (headerName.equalsIgnoreCase(webScannerDTO.getDescription())) {
						if (className == Integer.class) {
							value = (webScannerDTO.getValue() != null ? Integer.parseInt(webScannerDTO.getValue()) : null);
						}
					}
				}
			}

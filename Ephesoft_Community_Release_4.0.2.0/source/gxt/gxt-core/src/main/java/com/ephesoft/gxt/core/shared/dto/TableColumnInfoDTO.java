...
import com.ephesoft.gxt.core.shared.comparator.SortingProperty;
import com.ephesoft.gxt.core.shared.util.CollectionUtil;
import com.google.gwt.user.client.rpc.IsSerializable;

public class TableColumnInfoDTO implements IsSerializable, SortingProperty, Selectable, Comparable<TableColumnInfoDTO> {
	...
	private List<TableColumnDBExportMappingDTO> dbExportMappingList;
	...
	public void add(TableColumnDBExportMappingDTO mappingDTO) {
		if (dbExportMappingList == null) {
			dbExportMappingList = new LinkedList<TableColumnDBExportMappingDTO>();
		}
		dbExportMappingList.add(mappingDTO);
	}

	public void remove(TableColumnDBExportMappingDTO mappingDTO) {
		if (null != dbExportMappingList && null != mappingDTO) {
			dbExportMappingList.remove(mappingDTO);
			TableColumnDBExportMappingDTO newMappingDTO = new TableColumnDBExportMappingDTO();
			newMappingDTO.setId(mappingDTO.getId());
			newMappingDTO.setDeleted(true);
			newMappingDTO.setNew(mappingDTO.isNew());
			dbExportMappingList.add(newMappingDTO);
		}
	}
	...
	public List<TableColumnDBExportMappingDTO> getDbExportMappingList() {
		List<TableColumnDBExportMappingDTO> mappingList = new ArrayList<TableColumnDBExportMappingDTO>();
		if (!CollectionUtil.isEmpty(dbExportMappingList)) {
			for (TableColumnDBExportMappingDTO mapping : dbExportMappingList) {
				if (null != mapping && !mapping.isDeleted()) {
	...
	public List<TableColumnDBExportMappingDTO> getDbExportMappingList(boolean includeDeleted) {
		return includeDeleted ? dbExportMappingList : getDbExportMappingList();
	}

	public void setDbExportMappingList(List<TableColumnDBExportMappingDTO> dbExportMappingList) {
		this.dbExportMappingList = dbExportMappingList;
	}
	
	@Override
	public int compareTo(final TableColumnInfoDTO columnInfoDTO) {
		int comparisonResult = 0;
		if (null != columnInfoDTO) {
			if (columnName != null && columnName != null) {
				comparisonResult = columnName.compareTo(columnInfoDTO.columnName);
			}
		}

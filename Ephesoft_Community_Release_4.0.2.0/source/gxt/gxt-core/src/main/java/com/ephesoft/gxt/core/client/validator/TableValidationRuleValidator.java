...
/**
 * This validator is used to validate table validation rule.
 * 
 * @author Ephesoft
 * @version 1.0
 * @param <T> the generic type
 * @see com.ephesoft.gxt.core.client.validator.TableValidationRuleValidator
 */
public class TableValidationRuleValidator<T extends RuleInfoDTO> implements Validator<T> {
	...
	public boolean validate(List<T> listOfModels, T model, ValueProvider<T, ?> valueProvider) {
		...
		if (null != model && null != valueProvider) {
			...
			{
			isValid = ValidationUtil.isValidRule(valueToCheck);
			}
		}
		return isValid;
	}
	...
	public String getErrorMessage() {
		return LocaleDictionary.getConstantValue(LocaleCommonConstants.TOOLTIP_INCORRECT_RULE);
	}

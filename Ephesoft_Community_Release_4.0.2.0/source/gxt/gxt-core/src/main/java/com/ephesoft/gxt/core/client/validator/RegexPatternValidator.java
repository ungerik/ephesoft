 ...
 */
public class RegexPatternValidator<T> implements Validator<T> {
	...
	public RegexPatternValidator(boolean isValid) {
	...
	public boolean validate(List<T> listOfModels, T model, ValueProvider<T, ?> valueProvider) {
		...
		if (null != model && null != valueProvider) {
			...
			valueToCheck = valueToCheck == null ? CoreCommonConstant.EMPTY_STRING : valueToCheck;
			isValid = ValidationUtil.isRegexPatternValid(valueToCheck);
		}
		return isValid;
	}
	...
	public String getErrorMessage() {
		return LocaleDictionary.getConstantValue(LocaleCommonConstants.TOOLTIP_REGEX_NOT_VALID);
	}

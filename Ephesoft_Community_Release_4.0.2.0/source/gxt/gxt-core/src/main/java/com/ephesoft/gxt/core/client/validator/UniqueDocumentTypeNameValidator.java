...
/**
 * This validator is used to validate uniqueness of documentType
 * 
 * @author Ephesoft
 * @version 1.0
 ...
 */
public class UniqueDocumentTypeNameValidator<T extends DocumentTypeDTO, V> implements Validator<DocumentTypeDTO> {
	 ...
	 */
	@Override
	public boolean validate(List<DocumentTypeDTO> listOfModels, DocumentTypeDTO model,
			ValueProvider<DocumentTypeDTO, ?> valueProvider) {
		boolean valid = true;
		if (null != listOfModels && !listOfModels.isEmpty() && null != model && null != valueProvider && model.getName() != null
				&& !model.getName().isEmpty()) {
			for (DocumentTypeDTO documentDTOModel : listOfModels) {
				if (model != documentDTOModel && model.getName().trim().equalsIgnoreCase(documentDTOModel.getName())) {
					valid = false;
					break;
				}
			}
	...
	public String getErrorMessage() {
		return "DocumentType name must be unique and nonempty";

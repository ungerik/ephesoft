 ...
 */
public class UniqueEmailUserValidator<T extends EmailConfigurationDTO, V> implements Validator<EmailConfigurationDTO> {
	...
	public boolean validate(List<EmailConfigurationDTO> listOfModels, EmailConfigurationDTO model,
		...
		boolean valid = true;
		if (null != listOfModels && !listOfModels.isEmpty() && null != model && null != valueProvider && model.getUserName() != null
				&& !model.getUserName().isEmpty()) {
			for (EmailConfigurationDTO emailDTOModel : listOfModels) {
				if (model != emailDTOModel && model.getUserName().trim().equals(emailDTOModel.getUserName())) {
					valid = false;
					break;
				}
			}
	...
	public String getErrorMessage() {
		return "Username must be unique and nonempty";

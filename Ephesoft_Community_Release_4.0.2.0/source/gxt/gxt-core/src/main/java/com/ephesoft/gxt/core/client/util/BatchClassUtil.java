...
public class BatchClassUtil {
	...
	private BatchClassUtil() {
		// private constructor added to make the class non-instantiable.
	}

	public static boolean isDefaultBatchClass(BatchClassDTO batchClassIdentifier) {
		boolean isDefaultBatchClass = false;
		if (batchClassIdentifier != null) {
			isDefaultBatchClass = DefaultBatchClasses
					.isDefaultBatchClass(batchClassIdentifier.getIdentifier());
		}
		return isDefaultBatchClass;
	}

	public static boolean isDefaultBatchClass(List<BatchClassDTO> batchClassList) {
		boolean isDefaultBatchClass = false;
		if (batchClassList != null) {
			for (BatchClassDTO batchClassDTO : batchClassList) {
				isDefaultBatchClass = isDefaultBatchClass(batchClassDTO);
				if (!isDefaultBatchClass) {
					break;
				}
			}
		} else {

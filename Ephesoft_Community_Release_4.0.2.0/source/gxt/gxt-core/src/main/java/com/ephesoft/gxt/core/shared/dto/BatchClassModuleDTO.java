...
public class BatchClassModuleDTO implements IsSerializable {
	...
	public void addBatchClassPlugin(BatchClassPluginDTO batchClassPluginDTO) {
		if (!(batchClassPluginDTO != null && batchClassPluginDTO.getPlugin() != null && batchClassPluginDTO.getPlugin().isDeleted())) {
	...
	/**
	 * Gets the Non-Deleted Plugin by name <code>name</code>.
	 * @param name {@link String} name of the plugin.
	 * @return {@link BatchClassPluginDTO} from the list of non deleted plugins with name <code>name</code>. 
	 */
	public BatchClassPluginDTO getNonDeletedPluginByName(final String name) {
		BatchClassPluginDTO namedPluginDTO = null;
		for (BatchClassPluginDTO batchClassPluginDTO : batchClassPluginsMap.values()) {
			if (batchClassPluginDTO.getPlugin().getPluginName().equals(name) && !batchClassPluginDTO.isDeleted())
				namedPluginDTO = batchClassPluginDTO;
		}
		return namedPluginDTO;
	}

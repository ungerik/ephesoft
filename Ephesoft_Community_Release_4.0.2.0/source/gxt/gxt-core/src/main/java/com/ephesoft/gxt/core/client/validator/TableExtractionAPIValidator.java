...
/**
 * This validator is used to validate empty value.
 * 
 * @author Ephesoft
 * @version 1.0
 * @param <T> the generic type
 * @see com.ephesoft.gxt.core.client.validator.EmptyValueValidator
 */
public class TableExtractionAPIValidator<T extends TableExtractionRuleDTO, V extends TableExtractionAPIModel> implements Validator<T> {

	// specify whether empty validation is required for that field.
	...
	public boolean validate(List<T> listOfModels, T model, ValueProvider<T, ?> valueProvider) {
		...
		if (null != model && null != valueProvider) {
			...
			TableExtractionAPIModel apiModel = null;
			if (value != null) {
				apiModel = (TableExtractionAPIModel)value;
				if(apiModel != null && (apiModel.isColumnCoordinates() || apiModel.isColumnHeader() || apiModel.isRegexExtraction()))
				{
					isValid = true;
				}
			}
	...
	public String getErrorMessage() {
		return LocaleDictionary.getConstantValue(LocaleCommonConstants.TOOLTIP_TABLE_EXTRACTION_API); 
	}

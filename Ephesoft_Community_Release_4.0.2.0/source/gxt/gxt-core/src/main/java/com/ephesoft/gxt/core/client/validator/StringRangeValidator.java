...
public class StringRangeValidator<T> implements Validator<T> {
	...
	public StringRangeValidator(String errorMessage, Integer minVal,
			Integer maxVal, boolean isEmptyValueAllowed) {
		...
		if (null != model && null != valueProvider) {
			...
			if (null != value) {
				...
				if (null != valueToCheck) {
					int stringSize = valueToCheck.length();
					isValidNumber = ValidationUtil.isValueInRange(stringSize,
							minValue, maxValue);
				}
			} else if (null == value && isEmptyValueAllowed) {
				isValidNumber = true;
			}
		}

...
//			folderImporter.convertValidOfficeFiles(batchInstance, folderPath);
			folderImporter.breakMultiPagePDFandTiff(batchInstance, folderPath);
		} catch (Exception ex) {
			String message = ex.getMessage();
			LOGGER.error(message, ex);
			throw new DCMAException("Exception in breaking the input file. " + message, ex);
		}

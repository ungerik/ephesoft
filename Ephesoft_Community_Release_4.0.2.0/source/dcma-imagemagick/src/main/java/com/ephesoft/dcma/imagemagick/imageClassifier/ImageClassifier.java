...
public class ImageClassifier {
	...
	public void classifyAllImgsOfBatch(String batchInstanceIdentifier, String sBatchFolder) throws IOException,
		...
		String imMetric = pluginPropertiesService.getPropertyValue(batchInstanceIdentifier,
	...
	private Map<String, CustomValueSortedMap> performClassification(String maxVal, String imMetric, String imFuzz,
			String sampleBaseFolderPath, List<String> listOfUnclasifiedPgPth) throws IOException, DCMAApplicationException {
	 ...
	 * @throws IOException
	 */
	public void classifyAllImgsOfBatchInternal(String maxVal, String imMetric, String imFuzz, String batchInstanceIdentifier,
			String batchClassIdentifier, String sBatchFolder, List<Page> listOfPages) throws IOException, DCMAApplicationException {
	...
	private void updateXmlObject(Map<String, CustomValueSortedMap> finalUnclasifiedPageConfidenceMap,
		...
		for (String unclassifiedPage : unclasifiedPgsSet) {
			...
			pageLevelField.setConfidence(0.0f);
			pageLevelField.setFieldOrderNumber(0);
			pageLevelField.setForceReview(false);
			pageLevelField.setOcrConfidence(0.0f);
			pageLevelField.setOcrConfidenceThreshold(0.0f);
//			pageLevelField.setCategory("Uncategorised");
			pageLevelField.setCategory(CategoryType.GROUP_1.getCategoryName());
			...
			if (listOfConfidenceValues.size() >= 1) {
				...
				if (listOfConfidenceValues.last().getValue() == 0) {
					pageLevelField.setValue(ImageMagicKConstants.EMPTY);
				} else {
	 ...
	 * @throws IOException
	 */
	private double getMaxCondifence(final String unclasifiedPagePath, List<String> listOfSambleThumbsPaths, final String imMetric,
			final String imFuzz) throws IOException, DCMAApplicationException {
	...
	private List<String> getListOfThumbnailPaths(final String samplePageType, final String documentType,
		...
		if (thumbnailFolder.exists() && thumbnailFolder.isDirectory()) {
			String[] arrayOfThumbnailNames = null;
			int retry = 0;
			while (retry++ < 5) {
				arrayOfThumbnailNames = thumbnailFolder.list();
				if (null != arrayOfThumbnailNames) {
					for (final String nameOfSampleThumbnail : arrayOfThumbnailNames) {

...
public class StringUtil {
	 ...
	 */
	public static String getHiddenPasswordString(final String password) {
		String hiddenPasswordString = "";
		if (password != null) {
			hiddenPasswordString = password.replaceAll(CoreCommonConstants.ALL_CHARACTER_REGEX_STRING,
					CoreCommonConstants.PASSWORD_HIDDEN_STRING);
		}
		return hiddenPasswordString;
	}
			...
			for (int index = 0; index < words.length; index++) {
				...
				for (int charIndex = 0; charIndex < charsToBeReplaced.length; charIndex++) {
					if (words[index] == charsToBeReplaced[charIndex]) {
						resultStringBuilder.append(CoreCommonConstants.EMPTY_STRING);
						matchFailed = false;
						break;
					}
				}

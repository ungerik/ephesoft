...
public class ProcessExecutor extends AbstractRunnable {
	...
	/**
	 * The multipage image on which the process needs to be executed.
	 */
	private File imagePath;

	/**
	 * The boolean value indicates whether or not to delete the image post process.
	 */
	...
	public ProcessExecutor(String[] cmds, File environment) {
		this(cmds, environment, null, false);
	}
	...
	public ProcessExecutor(String[] cmds, File environment, boolean retry) {
		this(cmds, environment, null, false, retry);
	}
	 ...
	 * @param cmds the command that is to be executed.
	 * @param environment the environment in which it needs to be executed.
	 * @param imagePath the path of the image being used
	 * @param deleteImage boolean to decide whether we want to delete an image
	 */
	public ProcessExecutor(String[] cmds, File environment, File imagePath, boolean deleteImage) {
		this(cmds, environment, imagePath, deleteImage, false);
	}

	public ProcessExecutor(String[] cmds, File environment, File imagePath, boolean deleteImage, boolean retry) {
		super();
		if (cmds != null) {
			...
			if (exitValue != 0) {
				...
				if (retry) {
					...
					try {
						...
						String property = applicationConfigProperties.getProperty(ICommonConstants.MAXIMUM_NUMBER_OF_RETRIES);
					...
					if (numberOfRetries++ < maximumNumberOfRetries) {
						LOG.error("Retrying executing the command.");
						run();
					} else {
						throwNonZeroExitValueError();
					}
				} else {
					throwNonZeroExitValueError();
				}
				...
				if (deleteImage) {
					LOG.info("Deleting image at " + imagePath.getAbsolutePath() + " post conversion.");
					imagePath.delete();
				}
			}
		} catch (IOException e) {
	...
	private void throwNonZeroExitValueError() {

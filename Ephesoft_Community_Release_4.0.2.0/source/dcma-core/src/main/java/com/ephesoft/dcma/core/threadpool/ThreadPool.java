...
/**
 * Creates the thread in the thread pool and maintains the information of the threads per batch instance.
 * 
 * @author Ephesoft
 * 
...
public final class ThreadPool extends ThreadPoolExecutor {
	...
	/**
	 * Ascertains the permissible core pool size. It returns the lower of specified and configured pool size.
	 * 
	 * @param corePoolSize specified core pool size
	 * @return the minimum of the specified and the pool size specified in properties file.
	 */
	public int getMinCorePoolSize(int corePoolSize) {
		int defaultCorePoolSize = CORE_POOL_SIZE;
		int configuredCorePoolSize = getConfiguredPoolSize(false, defaultCorePoolSize);
		int minCorePoolSize = Math.min(corePoolSize, configuredCorePoolSize);
	 ...
	 * 
	 * @param isUsingGhostScript {@link boolean} creating thread pool for Ghostscript
	 * @return corePoolSize {@link int} size of thread pool
	 * 
	 */
	...
	/**
	 * Returns the configured thread pool size from properties file.
	 * 
	 * @param isUsingGhostScript
	 * @param defaultCorePoolSize
	 * @return configured thread pool size
	 */
	private int getConfiguredPoolSize(boolean isUsingGhostScript, int defaultCorePoolSize) {
	...
	/**
	 * Checks for maximum available CPU CORE COUNT. Throws an exception if the available count is null or ZERO.
	 * 
	 * @throws DCMAApplicationException , if CPU CORE COUNT is null.
	 */
	public Integer getAvailableCpuCount() throws DCMAApplicationException {
		return 2;
	}

...
public class SearchFiles {
	...
	public static Map<String, ImageDetails> generateConfidence(String indexFolder, String query, String field, int noOfPages, int maxValue)
			throws Exception {
		...
		try {
			...
			if (scoreDocs != null && scoreDocs.length > 0) {
				for (ScoreDoc scoreDoc : scoreDocs) {
					...
					/* Explanation exp = */searcher.explain(searchQuery, scoreDoc.doc);
					String docPageType = fetchDocPageType(document.get("path"));
					String [] fileDetailsParts = docPageType.split(";");
					//docNameScore.put(fileDetailsParts[0], new ImageDetails(fileDetailsParts[1], scoreDoc.score));
					docNameScore.add(fileDetailsParts[0], Double.valueOf(scoreDoc.score), fileDetailsParts[1]);
				}
			}
		} finally {
			if (reader != null) {
		...
		if (absolutePath != null && absolutePath.length() > 0) {
			...
			if (tokens != null && tokens.length > 0) {
				for (int i = 0; i < tokens.length; i++) {
					if (tokens[i].contains(".xml") || tokens[i].contains(".html")) {
						index = i;
					}
				}
			} else {
			...
			if (index >= 2) {
				returnValue = EphesoftStringUtil.concatenate(tokens[index - 1], ";", tokens[index]);
			} else {

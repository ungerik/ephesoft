...
public class TesseractReader implements ICommonConstants {
	 ...
	 */
	public void readOCR(final String batchInstanceID, String pluginName) throws DCMAApplicationException, DCMABusinessException, DCMAException {
		...
		if (("ON").equalsIgnoreCase(tesseractSwitch)) {
			...
			createHocrFiles(batchInstanceID);
			
			List<Document> docList = batch.getDocuments().getDocument();
			for (Document document : docList) {
				List<Page> pageList = document.getPages().getPage();
				for (Page page : pageList) {
					String hocrFileName = batchInstanceID + ICommonConstants.UNDERSCORE +page.getIdentifier() + FileType.HOCR_XML.getExtension();
					page.setHocrFileName(hocrFileName);
				}
			}
	...
	private void createHocrFiles(String batchInstanceIdentifier) throws DCMAApplicationException, DCMABusinessException, DCMAException {
		batchSchemaService.htmlToXmlGeneration(batchInstanceIdentifier);
	}

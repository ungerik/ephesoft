...
public class CustomValueSortedMap extends TreeSet<CustomMapClass> {
	...
	public boolean add(String key, Double value, String fileName) {
		CustomMapClass customMapClass = new CustomMapClass(key, value, fileName);
		return this.add(customMapClass);
	}
	...
	public Map<String, ImageDetails> getReverseSortedMapValueInFloat() {
		Map<String, ImageDetails> linkedHashMap = new LinkedHashMap<String, ImageDetails>();
		Iterator<CustomMapClass> itr = super.descendingIterator();
		while (itr.hasNext()) {
			CustomMapClass customMapClass = (CustomMapClass) itr.next();
			Float doubleValue = null;
			if(null != linkedHashMap.get(customMapClass.getKey())){
				doubleValue = linkedHashMap.get(customMapClass.getKey()).getConfidenceValue();
			}
			Float newValue = (float) ((double) customMapClass.getValue());
			if (doubleValue == null) {
				linkedHashMap.put(customMapClass.getKey(), new ImageDetails(customMapClass.getFileName(), newValue));
			} else if (doubleValue.compareTo(newValue) < 0) {
				linkedHashMap.put(customMapClass.getKey(), new ImageDetails(customMapClass.getFileName(), newValue));
			}
		}

...
public class TIFFUtil {
	...
	/**
	 * The <code>getSelectedTiffFile</code> method is used to limit the file to the page limit given.
	 * 
	...
	public static void getSelectedTiffFile(final File tiffFile, final int pageLimit) throws IOException {
		...
		if (null != tiffFile && getTIFFPageCount(tiffFile.getAbsolutePath()) > pageLimit) {
			try {
				...
				final String finalPath = tiffFile.getParent() + File.separator + name + System.currentTimeMillis()
						+ IUtilCommonConstants.EXTENSION_TIF;
	...
	/**
	 * Gets the image file dimensions. Currently only tiff, tif and png images are allowed.
	 * 
	 * @param imagePath {@link String} absolute image file path
	 * @throws Throws UnsupportedOperationException if any other file type other than tif, tiff and png files are passed.
	 * 
	 * @return returns the image dimension as an array, with width being the fist element and height second. If error occurs while
	 *         getting the dimensions, null is returned.
	 */
	public static int[] getImageDimensions(final String imagePath) {
		LOG.debug("Entering method getImageDimensions....");
		int[] imageDimension = null;
		FileType fileType = null;
		RandomAccessFileOrArray randomAccessFile = null;
		String fileExtension = FileUtils.getFileExtension(imagePath);

		if (!EphesoftStringUtil.isNullOrEmpty(fileExtension)) {
			LOG.debug(EphesoftStringUtil.concatenate("Getting dimensions for file = ", imagePath));
			...
			try {
				...
				switch (fileType) {
					...
					case TIF:
					case TIFF:
						randomAccessFile = new RandomAccessFileOrArray(imagePath);
						image = TiffImage.getTiffImage(randomAccessFile, 1);
						break;

					default:
						String errorMessage = EphesoftStringUtil.concatenate(
								"Invalid file type. Only tiff and png files are allowed. Input file = ", imagePath);
						LOG.debug(errorMessage);
						throw new UnsupportedOperationException(errorMessage);
				}
				imageDimension = new int[2];
				imageDimension[0] = (int) image.getWidth();
				imageDimension[1] = (int) image.getHeight();

			} catch (IOException ioException) {
				LOG.error(EphesoftStringUtil.concatenate("IO Exception occurred while getting the file dimensions for image.",
						imagePath), ioException);
			} catch (Exception exception) {
				LOG.error(
						EphesoftStringUtil.concatenate("Fatal error occurred while getting the file dimensions for image.", imagePath),
						exception);
			} finally {
				FileUtils.closeStream(randomAccessFile);
			}
		}
		LOG.debug("Exiting method getImageDimensions....");

...
//						docFieldType.setCategory("Uncategorised");
						...
						for (int k = 0; k < barCodeResults.length; k++) {
							if (k == 0) {
								...
								if (!value.isEmpty()) {
									docFieldType.setConfidence(Float.parseFloat(maxConfidence));
								} else {
								...
								fieldType.isForceReview();
								fieldType.setName(barcodeName + (k + 1));
								String value = barCodeResults[k].getTexts();
								fieldType.setValue(value);
								fieldType.setType(barCodeResults[k].getBarcodeType().name());
								if (!value.isEmpty()) {
									fieldType.setConfidence(Float.parseFloat(maxConfidence));
								} else {
									fieldType.setConfidence(Float.parseFloat(minConfidence));
								}
								Coordinates coordinates = new Coordinates();
								coordinates.setX0(new BigInteger(String.valueOf((int) barCodeResults[k].getX0Cord())));
								coordinates.setX1(new BigInteger(String.valueOf((int) barCodeResults[k].getX1Cord())));
			...
			for (int i = 0; i < allPages.size(); i++) {
				...
				if (validExtensions != null && validExtensions.length > 0) {
					for (int l = 0; l < validExtensions.length; l++) {
						if (eachPage.substring(eachPage.lastIndexOf(DOT_DELIMITER) + BARCODE_FIELD_SUFFIX).equalsIgnoreCase(
								validExtensions[l])) {
							isFileValid = true;
							break;
						}

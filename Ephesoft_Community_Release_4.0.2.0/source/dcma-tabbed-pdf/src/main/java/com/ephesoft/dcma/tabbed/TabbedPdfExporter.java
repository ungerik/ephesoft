...
public class TabbedPdfExporter implements ICommonConstants {
	...
	/**
	 * This will fetch the custom dlf's values.
	 * 
	 * @param documentList{@link List<Document>}
	 * @return {@link Map}<{@link String}, {@link String}>
	 */
	private Map<String, String> getTabbedPdfName(final List<Document> documentList) {
		Map<String, String> resultMap = null;
		if (CollectionUtils.isNotEmpty(documentList)) {
			for (Document document : documentList) {
				if (null != resultMap && resultMap.size() == TabbedPdfConstant.MAX_NAME_FIELDS) {
					break;
				}
				if (null != document) {
					...
					if (null != docFields) {
						...
						if (CollectionUtils.isNotEmpty(docFieldList)) {
							resultMap = getDLFNames(docFieldList);
						}
					}
				}
			}
	...
	/**
	 * Gets matched dlf names for a document's document level fields.
	 * 
	 * @param docFieldList {@link List}<{@link DocField}>
	 * @return {@link Map}<{@link String}, {@link String}>
	 */
	private Map<String, String> getDLFNames(final List<DocField> docFieldList) {
		Map<String, String> resultMap = null;
		for (DocField docField : docFieldList) {
			if (null != docField) {
				String docFieldName = docField.getName();
				if (TabbedPdfConstant.SUBPOENA.equals(docFieldName) || TabbedPdfConstant.LOAN_NUMBER.equals(docFieldName)) {
					if (null == resultMap) {
						resultMap = new HashMap<String, String>(TabbedPdfConstant.MAX_NAME_FIELDS);
					}
					resultMap.put(docFieldName, docField.getValue());
				}
			}
		}
		return resultMap;

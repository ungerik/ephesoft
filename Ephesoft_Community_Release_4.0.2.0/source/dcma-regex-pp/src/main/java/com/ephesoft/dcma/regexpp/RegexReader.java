...
//		docFieldType.setCategory("Uncategorised");
	...
	private void retrieveInputCarrierList(final BatchInstanceID batchInstanceID, final Map<String, List<InputDataCarrier>> mapCarrier,
			boolean isWebServiceCall) throws DCMAApplicationException {
		String errMsg = null;
		BatchPluginConfiguration[] batchClassPluginConfigList = null;
		if (isWebServiceCall) {
			batchClassPluginConfigList = batchClassPluginPropertiesService.getPluginProperties(batchInstanceID.getID(),
					KV_PAGE_PROCESS_PLUGIN, RegexProperties.KEY_VALUE_PATTERNS);
		} else {
			batchClassPluginConfigList = pluginPropertiesService.getPluginProperties(batchInstanceID.getID(), KV_PAGE_PROCESS_PLUGIN,
	...
	public void readRegex(final BatchInstanceID batchInstanceID) throws DCMAApplicationException, DCMAException {
		...
		if (regexSwitch.equalsIgnoreCase(ON_SWTICH)) {
			...
			retrieveInputCarrierList(batchInstanceID, mapCarrier, false);
			...
			for (Document document : documentTypeList) {
				...
				for (Page page : pageList) {
					...
					final HocrPages hocrPages = batchSchemaService.getHocrPages(batchInstanceID, page.getHocrFileName());
					final String ocrInputFileName = EphesoftStringUtil.concatenate(batch.getBatchLocalPath(), File.separatorChar,
							batchInstanceIdentifier, File.separatorChar, page.getOCRInputFileName());
	 ...
	 * This method is used to update the batch.xml file where the
	 * 
	 * @param batchClassID {@link String} identifier of batch class for which it has to run.
	 * @param xmlDocuments {@link List <Document>} list of documents.
	 * @param batchLocalPath {@link String} the path to the HOCR files.
	 * @throws DCMAException if any error occur in reading and updating batch.xml
	 * @throws DCMAApplicationException
	 */
	public void readRegexAPI(final String batchClassID, final List<Document> xmlDocuments, final String inputFilesPath)
			throws DCMAApplicationException, DCMAException {
		String regexSwitch = batchClassPluginPropertiesService.getPropertyValue(batchClassID, KV_PAGE_PROCESS_PLUGIN,
				RegexProperties.KVPAGEPROCESS_SWITCH);
		if (regexSwitch.equalsIgnoreCase(ON_SWTICH)) {
			String errMsg = null;
			LOGGER.info("Initializing......");
			String maxValue = batchClassPluginPropertiesService.getPropertyValue(batchClassID, KV_PAGE_PROCESS_PLUGIN,
					RegexProperties.KVPAGEPROCESS_MAX_RESULT_COUNT);
			int maxVal = 10;
			try {
				maxVal = Integer.parseInt(maxValue);
			...
			if (null == batchClassID) {
				errMsg = "Invalid batch class identifier.";
				LOGGER.error(errMsg);
				throw new DCMAApplicationException(errMsg);
			}
			...
			BatchInstanceID batchInstanceID = new BatchInstanceID(batchClassID);
			retrieveInputCarrierList(batchInstanceID, mapCarrier, true);

			List<OutputDataCarrier> outputDataCarriers = new ArrayList<OutputDataCarrier>();

			if (xmlDocuments == null) {
				errMsg = "Invalid Document List.";
				LOGGER.error(errMsg);
				throw new DCMAApplicationException(errMsg);
			}
			...
			for (Document document : xmlDocuments) {
				...
				for (Page page : pageList) {
					...
					final HocrPages hocrPages = batchSchemaService.getHocrPagesForTestContent(inputFilesPath, page.getHocrFileName());
					final String ocrInputFileName = EphesoftStringUtil.concatenate(inputFilesPath, File.separatorChar, page
							.getOCRInputFileName());
	...
	public void setBatchClassPluginPropertiesService(PluginPropertiesService batchClassPluginPropertiesService) {
		this.batchClassPluginPropertiesService = batchClassPluginPropertiesService;
	}

	public PluginPropertiesService getBatchClassPluginPropertiesService() {
		return batchClassPluginPropertiesService;
	}

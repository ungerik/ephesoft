...
public class ExtractDocField {
	...
	public void updateBatchXML(final String batchInstanceId, final List<List<DocField>> updtDocList, Batch batch,
			List<Boolean> isValidateDataList, BatchSchemaService batchSchemaService, boolean isBatchExecution)
		...
		if (isBatchExecution) {
			// now write the state of the object to the xml file if execution is coming from batch processing and not from test extraction.
			if (isWrite) {

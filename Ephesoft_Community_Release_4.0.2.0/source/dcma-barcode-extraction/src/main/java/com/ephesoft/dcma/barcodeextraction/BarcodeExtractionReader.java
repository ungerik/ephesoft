...
public class BarcodeExtractionReader implements ICommonConstants {
	...
	private static final String BARCODE_EXTRACTION = "Barcode Extraction";
	...
	/**
	 * Instance of BatchClassService {@link BatchClassService}
	 */
	@Autowired
	...
	public List<DocField> updateBatchXML(final List<BarcodeExtractionExecutor> barcodeExecutorList, final Batch batch,
			final String maxConfidence, final String minConfidence, final String batchInstanceId, boolean isBatchExecution)
			throws DCMAApplicationException {
		...
		for (BarcodeExtractionExecutor barcodeExtractionExecutor : barcodeExecutorList) {
			...
			if (isBatchExecution) {
				...
				allFdTypes = fieldTypeService.getFdTypeByDocTypeNameForBatchClass(docTypeName, batchInstanceId);
			}
	...
	private DocField createDefaultDLF(final String minConfidence, FieldType fieldType) {
		...
		docFieldType.setConfidence(Float.parseFloat(minConfidence));
		if (null != fieldType.getCategoryName()) {
			docFieldType.setCategory(fieldType.getCategoryName());
		} else {
//			docFieldType.setCategory("Uncategorised");
			docFieldType.setCategory(CategoryType.GROUP_1.getCategoryName());
		}
		docFieldType.setHidden(fieldType.isHidden());
		docFieldType.setExtractionName(BARCODE_EXTRACTION);
	...
	private DocField populateDLFForBarcodeType(final String maxConfidence, final String minConfidence,
		...
		docField.setConfidence(0.0f);
		docField.setFieldOrderNumber(0);
		docField.setForceReview(false);
		docField.setOcrConfidence(0.0f);
		docField.setOcrConfidenceThreshold(0.0f);
		docField.setValue(barCodeResult.getTexts());
		docField.setName(fieldType.getName());
		docField.setExtractionName(BARCODE_EXTRACTION);
		docField.setType(fieldType.getDataType().name());
		docField.setFieldOrderNumber(fieldType.getFieldOrderNumber());
		if (null != fieldType.getCategoryName()) {
			docField.setCategory(fieldType.getCategoryName());
		} else {
//			docField.setCategory("Uncategorised");
			docField.setCategory(CategoryType.GROUP_1.getCategoryName());
		}
		docField.setHidden(fieldType.isHidden());
		Coordinates coordinates = new Coordinates();
		...
		if (docField != null && docField.getValue() != null && !docField.getValue().isEmpty()) {
			docField.setConfidence(Float.parseFloat(maxConfidence));
		} else {
			docField.setConfidence(Float.parseFloat(minConfidence));
		}
		...
		if (switchValue.equalsIgnoreCase(SWITCH_ON)) {
			...
			for (List<BarcodeExtractionExecutor> barcodeExecutor : barcodeDOCExecutorList) {
				...
				List<DocField> updtDocFdTyList = updateBatchXML(barcodeExecutor, batch, maxConfidence, minConfidence,
						batchInstanceIdentifier, true);
				updtDocList.add(updtDocFdTyList);
			}

			documentLevelFieldsMapping(batch, xmlDocuments, updtDocList, true);
		} else {
			LOGGER.info("Skipping barcode classification. Switch set as off.");
		}
	}

	public List<DocField> readBarcodeForWebService(final String batchClassIdentifier, String folderLocation, String imageName,
			String docType) throws DCMAApplicationException {
		LOGGER.info("Started Processing image at " + new Date());
		BatchClass batchClass = batchClassService.getBatchClassByIdentifier(batchClassIdentifier);
		List<DocumentType> validDocTypeList = batchClass.getDocumentTypes();
		boolean isValidDocType = false;
		for (DocumentType documentType : validDocTypeList) {
		...
		// Initialize properties
		LOGGER.info("Initializing properties...");
		String validExt = batchClassPluginPropertiesService.getPropertyValue(batchClassIdentifier, BARCODE_EXTRACTION_PLUGIN,
				BarcodeExtractionProperties.BARCODE_VALID_EXTNS);
		String readerTypes = batchClassPluginPropertiesService.getPropertyValue(batchClassIdentifier, BARCODE_EXTRACTION_PLUGIN,
				BarcodeExtractionProperties.BARCODE_READER_TYPES);
		String maxConfidence = batchClassPluginPropertiesService.getPropertyValue(batchClassIdentifier, BARCODE_EXTRACTION_PLUGIN,
				BarcodeExtractionProperties.MAX_CONFIDENCE);
		String minConfidence = batchClassPluginPropertiesService.getPropertyValue(batchClassIdentifier, BARCODE_EXTRACTION_PLUGIN,
				BarcodeExtractionProperties.MIN_CONFIDENCE);
		...
		if (isFileValid) {
			...
			BarcodeExtractionExecutor barcodeExecutor = new BarcodeExtractionExecutor(folderLocation + File.separator + imageName,
					imageName, appReaderTypes, null, null);
			barcodeExecutorList.add(barcodeExecutor);
			LOGGER.info("Done with Zxing library for image : " + imageName);
		} else {
			LOGGER.error("File " + imageName + " has invalid extension.");
			throw new DCMABusinessException("File " + imageName + " has invalid extension.");
		}
		List<DocField> updtDocFdTyList = new ArrayList<DocField>();
		List<FieldType> fieldTypeList = fieldTypeService.getFdTypeByDocTypeNameForBatchClass(docType, batchClassIdentifier);
	...
	/**
	 * Performs barcode extraction on the input image provided
	 * 
	 ...
	 * @throws DCMAApplicationException
	 */
	public void readBarcodeForExtraction(String batchClassIdentifier, String folderLocation, Batch batch)
			throws DCMAApplicationException {
		LOGGER.info("Started Processing image at " + new Date());
		...
		for (Document document : xmlDocuments) {
			...
			for (Page page : listOfPages) {
				String sImageFile = page.getOldFileName();
				if (sImageFile == null || sImageFile.isEmpty()) {
					continue;
				}
				String eachPage = sImageFile;
				...
				if (validExtensions != null && validExtensions.length > 0) {
					for (int l = 0; l < validExtensions.length; l++) {
						if (eachPage.substring(eachPage.lastIndexOf('.') + 1).equalsIgnoreCase(validExtensions[l])) {
							isFileValid = true;
							break;
						}
					}
				...
				if (isFileValid) {
					LOGGER.info("Calling Zxing library for image :" + eachPage);
					LOGGER.error("*************folder location is " + folderLocation);
					BarcodeExtractionExecutor barcodeExecutor = new BarcodeExtractionExecutor(folderLocation + File.separator
							+ eachPage, eachPage, appReaderTypes, page, document);
					barcodeExecutorList.add(barcodeExecutor);
					LOGGER.info("Done with Zxing library for image : " + eachPage);
				} else {
					LOGGER.error("File " + eachPage + " has invalid extension.");
		...
		for (List<BarcodeExtractionExecutor> barcodeExtractionExecutorList : barcodeDOCExecutorList) {
			for (BarcodeExtractionExecutor barcodeExtractionExecutor : barcodeExtractionExecutorList) {
				barcodeExtractionExecutor.run();
			}
		}
		List<List<DocField>> updtDocList = new ArrayList<List<DocField>>();
		...
		documentLevelFieldsMapping(batch, xmlDocuments, updtDocList, false);
	}

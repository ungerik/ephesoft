...
/**
 * This class does the actual processing on the basis of Automatic classification for document assembly. It will read all the pages of
...
public class AutomaticClassification implements DocumentClassification {
	...
	public final void processUnclassifiedPages(final DocumentAssembler documentAssembler, final String batchInstanceID,
		...
		String barcodeConfidence = pluginPropertiesService.getPropertyValue(batchInstanceID,
		...
		automaticPageProcess.setPluginPropertiesService(pluginPropertiesService);
		...
		String deleteDocFirstPageSwitch = pluginPropertiesService.getPropertyValue(batchInstanceID,
				DocumentAssemblerConstants.DOCUMENT_ASSEMBLER_PLUGIN, DocumentAssemblerProperties.DA_DELETE_DOCUMENT_FIRST_PAGE_SWITCH);
		...
		if (null != docPageInfo) {
			...
			automaticPageProcess.createDocForPages(insertAllDocument, docPageInfo, false);
		}

	}

	@Override
	public void processDocumentTypes(DocumentAssembler documentAssembler, String batchInstanceIdentifier, String batchClassIdentifer,
		...
		automaticPageProcess.setBatchInstanceID(batchInstanceIdentifier);
		automaticPageProcess.setBatchSchemaService(documentAssembler.getBatchSchemaService());
		automaticPageProcess.setDocumentTypeService(documentAssembler.getDocTypeService());
		automaticPageProcess.setBatchClassIdentifier(batchClassIdentifer);

		automaticPageProcess.reClassifyDocuments(documentList);
	...
	public List<Document> processUnclassifiedPagesAPI(List<Page> docPageInfo, DocumentAssembler documentAssembler,
			String batchClassID, PluginPropertiesService bcPluginPropertiesService) {
		LOGGER.info("Setting all the fields for LucenePageProcess.");

		AutomaticPageProcess automaticPageProcess = new AutomaticPageProcess();
		
		setProperties(documentAssembler, batchClassID, bcPluginPropertiesService, automaticPageProcess);

		List<Document> insertAllDocument = new ArrayList<Document>();
		try {
			automaticPageProcess.createDocForPages(insertAllDocument, docPageInfo, true);
	...
	private void setProperties(final DocumentAssembler documentAssembler, final String batchClassID,
			final PluginPropertiesService pluginPropertiesService, final AutomaticPageProcess automaticPageProcess) {
		automaticPageProcess.setPageTypeService(documentAssembler.getPageTypeService());
		automaticPageProcess.setPluginPropertiesService(pluginPropertiesService);
		automaticPageProcess.setBatchClassIdentifier(batchClassID);
		automaticPageProcess.setDocumentTypeService(documentAssembler.getDocTypeService());
		automaticPageProcess.setBatchSchemaService(documentAssembler.getBatchSchemaService());
		...
		LOGGER.info("Initializing properties...");
		String factoryClassification = DocumentClassificationFactory.AUTOMATIC.toString();
		...
		String mergeUnknownSwitch = pluginPropertiesService.getPropertyValue(batchClassID,
				DocumentAssemblerConstants.DOCUMENT_ASSEMBLER_PLUGIN, DocumentAssemblerProperties.DA_MERGE_UNKNOWN_DOCUMENT_SWITCH);
		String deleteDocFirstPageSwitch = pluginPropertiesService.getPropertyValue(batchClassID,
				DocumentAssemblerConstants.DOCUMENT_ASSEMBLER_PLUGIN, DocumentAssemblerProperties.DA_DELETE_DOCUMENT_FIRST_PAGE_SWITCH);
		...
		automaticPageProcess.setPropertyMap(propertyMap);
		//Assigning the default value of barcode confidence to be set in case of finding a barcode in a page.

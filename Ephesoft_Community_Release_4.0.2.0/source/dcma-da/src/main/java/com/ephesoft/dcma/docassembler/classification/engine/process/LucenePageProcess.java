 ...
 * classification and will create new documents and delete the current page from the document type Unknown.
 * 
 * @author Ephesoft
 * @version 1.0
...
public class LucenePageProcess {
	...
	/**
	 * {@link PageTypeService} Instance of service to access the attributes related to the {@link Page}.
	 */
	private PageTypeService pageTypeService;

	/**
	 * {@link PluginPropertiesService} Instance of service to access all the properties required for this plugin.
	 */
	private PluginPropertiesService pluginPropertiesService;

	/**
	 * {@link DocumentTypeService} Instance of service to access the attributes of a {@link Document}.
	 */
	private DocumentTypeService docTypeService;

	/**
	 * List of type <code>Document a list to hold all the documents created by classification of pages.
	 */
	private List<Document> xmlDocuments;

	/**
	 * {@link String} Batch instance ID.
	 */
	private String batchInstanceIdentifier;

	/**
	 * {@link String} Batch Class ID.
	 */
	private String batchClassIdentifier;

	/**
	 * {@link BatchSchemaService} Instance of service used to read and write the data into Batch.xml file.
	 */
	private BatchSchemaService batchSchemaService;

	/**
	 * {@link Map} <code>String, String</code> for keeping the mapping of the property with its value.
	 */
	private Map<String, String> propertyMap;

	/**
	 * @return {@link Map} the propertyMap
	 */
	...
	/**
	 * @param pageTypeService {@link PageTypeService} the pageTypeService to set
	 */
	...
	/**
	 * @return batchClassIdentifier {@link String}
	 */
	...
	/**
	 * @return the pluginPropertiesService {@link PluginPropertiesService}
	 */
	...
	/**
	 * @param docTypeService {@link DocumentTypeService} the doc type service to be set
	 */
	...
	/**
	 * Default confidence threshold for middle page.
	 */
	private float middlePageConfidenceThreshold = 0;
	/**
	 * Default confidence threshold for last page.
	 */
	private float lastPageConfidenceThreshold = 0;
	/**
	 * Default confidence threshold for first page.
	 */
	private float firstPageConfidenceThreshold = 0;

	/**
	 * Sets the document type for the page type name provided if that document is present in the batch class called by web
	 * service/other non batch functionality.
	 * 
	 * @param docType {@link Document} document created after classification for which document types is to be set.
	 * @param pageTypeName {@link String} the page type found and classified for the document to be set.
	 */
	...
	/**
	 * Sets the document type for the page type name provided if that document is present in the batch class called by batch prosessing
	 * functionality.
	 * 
	 * @param docType {@link Document} document created after classification for which document types is to be set.
	 * @param pageTypeName {@link String} the page type found and classified for the document to be set.
	...
	public void setDocTypeNameAndConfThreshold(final Document docType, final String pageTypeName) {
		...
		if (null == docTypes || docTypes.isEmpty()) {
			...
			while (itr.hasNext()) {
				...
				docTypeDesciption = docTypeDB.getDescription();
				minConfidenceThreshold = docTypeDB.getMinConfidenceThreshold();
				LOGGER.debug(EphesoftStringUtil.concatenate("DocumentType name : ", docTypeName, "  minConfidenceThreshold : ",
						minConfidenceThreshold));
				if (null != docTypeName) {
		...
		if (null != docTypeName) {
			docType.setType(docTypeName);
			docType.setDescription(docTypeDesciption);
			DecimalFormat twoDForm = new DecimalFormat("#.##");
			minConfidenceThreshold = Float.valueOf(twoDForm.format(minConfidenceThreshold));
			docType.setConfidenceThreshold(minConfidenceThreshold);
		} else {
	 ...
	 * 
	 * @param insertAllDocument List<DocumentType> List of all the documents created after classification to be updated in batch xml
	 *            file for web service calls.
	 * @param isFromWebService boolean true/false values to check the call from web service or batch processing functionality.
		...
		try {
			...
			if (null != predefinedDocType && !predefinedDocType.isEmpty()) {
				DocumentType predefinedDocTypeDesc = DocumentAssembler.getDescriptionForDocument(pluginPropertiesService,
						predefinedDocType, batchClassIdentifier, isFromWebService, docTypeService);
		...
		if (null != switchUnknownDocType && DocumentAssemblerConstants.DA_SWITCH_ON.equalsIgnoreCase(switchUnknownDocType)) {
			if (null != unknownDocType && !unknownDocType.isEmpty()) {
				DocumentType unknownDocTypeDesc = DocumentAssembler.getDescriptionForDocument(pluginPropertiesService, unknownDocType,
						batchClassIdentifier, isFromWebService, docTypeService);
		...
		for (int i = 0; i < insertAllDocument.size(); i++) {
			...
			if (documnetTypes != null && documnetTypes.size() == 2) {
				DocumentType documentDesc = DocumentAssembler.getDescriptionForDocument(pluginPropertiesService, documnetTypes.get(1)
						.getName(), batchClassIdentifier, isFromWebService, docTypeService);
	 ...
	 * 
	 * @param insertAllDocument List<DocumentType> List of all the documents created after classification to be updated in batch xml
	 *            file for batch processing calls.
	 * @param isFromWebService boolean true/false values to check the call from web service or batch processing functionality.
	 * @throws DCMAApplicationException Check for input parameters, update the batch xml.
	 */
	private void updateBatchXML(final List<Document> insertAllDocument, final List<Integer> removeIndexList)
		...
		try {
			...
			if (null != predefinedDocType && !predefinedDocType.isEmpty()) {
				DocumentType predefinedDocTypeDesc = DocumentAssembler.getDescriptionForDocument(pluginPropertiesService,
						predefinedDocType, batchInstanceIdentifier, false, null);
				if (null != predefinedDocTypeDesc) {
		...
		// set default document type for unknown documents if the switch is on.
		String switchUnknownDocType = getPropertyMap().get(DocumentAssemblerConstants.SWITCH_UNKNOWN_PREDEFINED_DOCUMENT_TYPE).trim();
		String unknownDocType = getPropertyMap().get(DocumentAssemblerConstants.UNKNOWN_PREDEFINED_DOCUMENT_TYPE);
		if (null != switchUnknownDocType && DocumentAssemblerConstants.DA_SWITCH_ON.equalsIgnoreCase(switchUnknownDocType)) {
			if (null != unknownDocType && !unknownDocType.isEmpty()) {
				DocumentType unknownDocTypeDesc = DocumentAssembler.getDescriptionForDocument(pluginPropertiesService, unknownDocType,
	...
	/**
	 * This method will retrieve the page level field type DocField for any input page for current classification name.
	 * 
	 * @param pgType {@link Page} the current page being processed.
	 * @return docFieldType {@link DocField} the value for lucene classification in page provided.
	 */
	 ...
	 * 
	 * @param xmlDocuments {@link List} list of all the documents.
	 * @param isFromWebService true/false value to check if the call is from a web service of from batch processing.
	 * @throws DCMAApplicationException
	 */
	private void setDocConfAndDocType(final List<Document> xmlDocuments, boolean isFromWebService) throws DCMAApplicationException {
		...
		for (Document docType : xmlDocuments) {
			...
			processDocument(docType, docConfidence, isFromWebService);
	...
	// The new algorithm for classification.....................
	...
	private void performDocCreationForPages(List<Document> insertAllDocument, List<Page> docPageInfo, boolean isFromWebService)
		...
		for (int index = 0; index < docPageInfo.size(); index++) {
			...
			if (null == docFieldType) {
				...
				errMsg.append(getPropertyMap().get(DocumentAssemblerConstants.LUCENE_CLASSIFICATION));
		...
		if (alternateValues != null) {
			...
			if (alternateValueList != null && alternateValueList.size() > 0) {
				for (int i = 0; i < alternateValueList.size(); i++) {
					...
					if (getPropertyMap().get(DocumentAssemblerConstants.LUCENE_CLASSIFICATION).equals(alternateValue.getName())) {
	...
	public void createDocForPagesOldAlgorithm(List<Document> insertAllDocument, List<Page> docPageInfo, boolean isFromWebService)
			throws DCMAApplicationException {
	...
	@SuppressWarnings("unchecked")
	private void processDocument(final Document docType, final Map<String, Object[]> docConfidence, final boolean isFromWebService)
			throws DCMAApplicationException {
	...
	/**
	 * This method does the reclassification for the documents which doesn't have document type defined. If document is already
	 * classified and has document type defined, confidence is set for the same.
	 * 
	 * @param documentList List<{@link Document}> documents list shich is to be reclassified
	 * @throws DCMAApplicationException
	 */

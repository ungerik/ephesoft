...
public class ServerRegistryDaoImpl extends HibernateDao<ServerRegistry> implements ServerRegistryDao {
	 ...
	 * 
	 * @return {@link List} list of server registry of all active servers
	 */
	@Override
	...
	public List<ServerRegistry> getActiveLicenseServers() {
		DetachedCriteria criteria = criteria();
		criteria.add(Restrictions.eq("active", true) );
		criteria.add(Restrictions.eq("licenseServer", true));
		return find(criteria);
	}
	
	/**
	 * Returns folderMoniter active server list
	 * 
	...
	public List<ServerRegistry> getActiveFolderMoniterServers() {
		DetachedCriteria criteria = criteria();
		criteria.add(Restrictions.eq("active", true) );
		criteria.add(Restrictions.eq("folderMoniterServer", true));
		return find(criteria);
	}

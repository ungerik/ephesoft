...
public class FieldType extends AbstractChangeableEntity implements Serializable {
	...
	@Column(name = "field_category_name")
	...
	@Column(name = "pattern", length = 700)
	...
	@Column(name = "field_value_script_status", columnDefinition = "bit default 0")
	private boolean isFieldValueChangeScriptEnabled;

	@Column(name = "ocr_confidence_threshold")
	...
	@javax.persistence.OrderBy("orderNo")
	...
	@Cascade({CascadeType.SAVE_UPDATE, CascadeType.DELETE_ORPHAN, CascadeType.MERGE, CascadeType.EVICT})
	@JoinColumn(name = "field_type_id")
	private List<IndexFieldDBExportMapping> dbExportMappings = new ArrayList<IndexFieldDBExportMapping>();

	/**
	 * regexPatternSeparator {@link String} specifies the regex listing logical separtaor. For example OR, AND etc.
	 */
	@Column(name = "regex_listing_separator")
	...
	public void add(final IndexFieldDBExportMapping indexFieldMapping) {
		if (null != indexFieldMapping) {
			if (null == dbExportMappings) {
				dbExportMappings = new ArrayList<IndexFieldDBExportMapping>();
			}
			dbExportMappings.add(indexFieldMapping);
		}
	}
	...
	/**
	 * API to set the boolean flag 'isReadOnly'
	 * 
	...
	/**
	 * API to get the boolean flag 'isReadOnly'
	 * 
	...
	/**
	 * Gets the isFieldValueChangeScriptEnabled.
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the isFieldValueChangeScriptEnabled.
	 * 
	 * @param isFieldValueChangeScriptEnabled
	 */
	 ...
	 * Sets OCR confidence.
	 * 
	 * @param ocrConfidenceThreshold {@link Float}
	 */
	 ...
	 */
	public List<IndexFieldDBExportMapping> getDbExportMappings() {
		return dbExportMappings;
	}

	public IndexFieldDBExportMapping getMappingByID(final long id) {
		IndexFieldDBExportMapping bindedField = null;
		if (!CollectionUtil.isEmpty(dbExportMappings)) {
			for (IndexFieldDBExportMapping mapping : dbExportMappings) {
				if (mapping != null && mapping.getId() == id) {
					bindedField = mapping;
					break;
				}
			}
	...
	public void removeMappingById(final long id) {
		if (!CollectionUtil.isEmpty(dbExportMappings)) {
			for (IndexFieldDBExportMapping mapping : dbExportMappings) {
				if (mapping != null && mapping.getId() == id) {
	 ...
	 */
	public void setDbExportMappings(List<IndexFieldDBExportMapping> dbExportMappings) {

...
public class BatchInstanceGroupsDaoImpl extends HibernateDao<BatchInstanceGroups> implements BatchInstanceGroupsDao {
	...
	@Transactional
	@Override
	public void deleteBatchInstanceInGroups(String batchInstanceIdentifier) {
		final DetachedCriteria criteria = criteria();
		final Disjunction disjunction = Restrictions.disjunction();
		disjunction.add(Restrictions.eq(BATCH_INSTANCE_ID, batchInstanceIdentifier));
		criteria.add(disjunction);
		final List<BatchInstanceGroups> batchClassGroups = find(criteria);
		if (batchClassGroups != null && batchClassGroups.size() > 0) {
			removeAll(batchClassGroups);
		}
	}
}

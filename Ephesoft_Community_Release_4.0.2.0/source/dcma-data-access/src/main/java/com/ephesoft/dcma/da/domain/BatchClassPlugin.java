...
public class BatchClassPlugin extends AbstractChangeableEntity implements Cloneable, Comparable<BatchClassPlugin> {
	...
	public void setBatchClassPluginConfigs(List<BatchClassPluginConfig> batchClassPluginConfigs) {
		List<BatchClassPluginConfig> pluginConfigs = new ArrayList<BatchClassPluginConfig>();
		if(batchClassPluginConfigs != null){
			for(BatchClassPluginConfig batchClassPluginConfig : batchClassPluginConfigs){
				if (!(batchClassPluginConfig != null && batchClassPluginConfig.getPluginConfig() != null
						&& batchClassPluginConfig.getPluginConfig().isDeleted() != null
						&& batchClassPluginConfig.getPluginConfig().isDeleted())) {
					pluginConfigs.add(batchClassPluginConfig);
				} 
			}
		}
		this.batchClassPluginConfigs = pluginConfigs;
	}
	...
	public BatchClassPluginConfig getBatchClassPluginConfigByName(String pluginConfigName) {
		BatchClassPluginConfig pluginConfig = null;
		if (getBatchClassPluginConfigs() != null && getBatchClassPluginConfigs().size() > 0) {
			for (BatchClassPluginConfig batchClassPluginConfig : getBatchClassPluginConfigs()) {
				if (batchClassPluginConfig.getName().equalsIgnoreCase(pluginConfigName)) {
					pluginConfig = batchClassPluginConfig;
					break;
				}
			}
	...
	public List<BatchClassPluginConfig> getBatchClassPluginConfigByDataType(DataType dataType) {
		List<BatchClassPluginConfig> batchClassPluginConfigs = null;
		if (this.batchClassPluginConfigs != null && this.batchClassPluginConfigs.size() > 0) {
			for (BatchClassPluginConfig batchClassPluginConfig : this.batchClassPluginConfigs) {
				if (batchClassPluginConfig.getPluginConfig().getDataType() == dataType) {
					if (batchClassPluginConfigs == null) {
						batchClassPluginConfigs = new ArrayList<BatchClassPluginConfig>();
					}
					batchClassPluginConfigs.add(batchClassPluginConfig);
				}
			}
		}
		return batchClassPluginConfigs;
	...
	/**
	 * Compare plugins on the basis of their order of execution in a batch class.
	 * 
	 * @param module {@link BatchClassPlugin}
	 */
	@Override
	public int compareTo(BatchClassPlugin plugin) {
		int result;
		if (orderNumber < plugin.getOrderNumber()) {
			result = -1;
		} else if (orderNumber > plugin.getOrderNumber()) {
			result = 1;
		} else {
			result = 0;
		}

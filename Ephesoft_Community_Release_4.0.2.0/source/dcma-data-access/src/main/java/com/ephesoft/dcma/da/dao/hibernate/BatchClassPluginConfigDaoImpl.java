...
public class BatchClassPluginConfigDaoImpl extends HibernateDao<BatchClassPluginConfig> implements BatchClassPluginConfigDao {
	...
	private static final String PLUGIN_CONFIG = "pluginConfig";
	...
	private static final String PLUGIN_CONFIG_ID = PLUGIN_CONFIG + ".id";
	private static final String PLUGIN_CONFIG_DATA_TYPE = PLUGIN_CONFIG + ".dataType";
	...
	@Override
	public List<BatchClassPluginConfig> getAllPluginPropertiesForBatchClassByQualifier(String batchClassIdentifier, String pluginName,
			String qualifier) {
		DetachedCriteria criteria = criteria();
		criteria.createAlias(BATCH_CLASS_PLUGIN, BATCH_CLASS_PLUGIN, JoinFragment.INNER_JOIN);
		criteria.createAlias(BATCH_CLASS_PLUGIN_BATCH_CLASS_MODULE, BATCH_CLASS_MODULE, JoinFragment.INNER_JOIN);
	...
	public List<BatchClassPluginConfig> getBatchClassPluginConfigurationForPluginConfigId(Long pluginConfigId) {
		DetachedCriteria criteria = criteria();
		criteria.createAlias(PLUGIN_CONFIG, PLUGIN_CONFIG, JoinFragment.INNER_JOIN);
	...
	public List<BatchClassPluginConfig> getBatchClassPluginConfigByDataType(DataType dataType) {
		DetachedCriteria criteria = criteria();
		criteria.createAlias(PLUGIN_CONFIG, PLUGIN_CONFIG, JoinFragment.INNER_JOIN);
		criteria.add(Restrictions.eq(PLUGIN_CONFIG_DATA_TYPE, dataType));
		return find(criteria);
	}

...
/**
 * The <code>ServiceStatusService</code> is an interface which defines services to user to create, update service status and fetch
 * service status details. These services are used to apply business rules for fail-over mechanism for Ephesoft services.
 * 
 * @author Ephesoft
 * @version 1.0
...
public interface ServiceStatusService {
	...
	/**
	 * Creates service status entry in service_status for a service to be running on the current server.
	 * 
	 * @param serviceStatus {@link ServiceStatus} service status instance to be added
	 */
	void createServiceStatus(ServiceStatus serviceStatus);

	/**
	 * Removes service status entry form service_status table usually done to remove service which doesn't need to be distributed among
	 * servers.
	 * 
	 * @param serviceStatus {@link ServiceStatus} service status instance to be removed
	 */
	void removeServiceStatus(ServiceStatus serviceStatus);

	/**
	 * Updates the service status when some other server is taking ownership of running a particular service.
	 * 
	 * @param serviceStatus {@link ServiceStatus} service status instance to be updated.
	 */
	void updateServiceStatus(ServiceStatus serviceStatus);
